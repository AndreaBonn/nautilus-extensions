...
DOCKERFILE_NAMES = [
    ...
    'Dockerfile.local', 'Dockerfile.staging',
]
...
def parse_dockerfile(path: str) -> dict:
    ...
    result['num_layers'] = sum(
        1 for ins in result['instructions']
        if ins['instruction'] in ('FROM', 'RUN', 'COPY', 'ADD')
    )
    ...
    for run in run_cmds:
        ...
        if 'pip install' in cmd and 'requirements' not in cmd:
            ...
            for pkg_str in pkgs:
                ...
                unversioned = [p for p in pkgs_list if p and '==' not in p and not p.startswith('-')]
                if unversioned:
                    warn('medium',
...
class DockerfileWindow(Gtk.Window):
    ...
    def _build_content(self, data: dict):
        ...
        self._stat(info_bar, "Porte",   str(len(data['exposed_ports'])) if data['exposed_ports'] else '—')
        ...
        notebook.append_page(self._tab_warnings(data),     Gtk.Label(label=f"⚠ Best Practice ({len(data['warnings'])})"))
    ...
    def _tab_overview(self, data: dict) -> Gtk.Widget:
        ...
        for i, stage in enumerate(data['stages']):
            ...
            img_lbl.set_markup(f"<span foreground='{INSTRUCTION_COLORS['FROM']}'><b>{stage['image']}</b></span>")
        ...
        if data['exposed_ports']:
            ...
            for port in data['exposed_ports']:
                ...
                chip.set_markup(f"<span foreground='{INSTRUCTION_COLORS['EXPOSE']}'><b>{port}</b></span>")
    ...
    def _tab_warnings(self, data: dict) -> Gtk.Widget:
        ...
        level_css   = {'high': 'warn-high', 'medium': 'warn-medium', 'low': 'warn-low', 'ok': 'warn-ok'}
...
class DockerfileExtension(GObject.GObject, Nautilus.MenuProvider):
    ...
    def get_background_items(self, folder):
        ...
        if folder is None:
            return []
        folder_path = unquote(folder.get_uri().replace('file://', ''))
