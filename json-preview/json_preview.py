...
def merge_schemas(schemas: list) -> dict:
    ...
    for key in all_keys:
        present = [s for s in schemas if s.get('type') == 'object'
                   and 'children' in s and key in s['children']]
        count = len(present)

        # Tipo più frequente per questo campo
        type_counts = collections.Counter(
            p['children'][key]['type'] for p in present
        )
...
class JsonPreviewWindow(Gtk.Window):
    ...
    def _build_content(self, data: dict):
        ...
        if merged and merged.get('type') == 'object' and merged.get('children'):
            notebook.append_page(
                self._tab_schema(merged),
                Gtk.Label(label="🗂 Schema")
            )
    ...
    def _value_preview(self, value) -> str:
        ...
        if t == 'boolean': return str(value).lower()
        if t == 'number':  return str(value)
        if t == 'string':
            s = str(value)
            return s[:80] + '…' if len(s) > 80 else s
        if t == 'object':  return f'{{ {len(value)} chiavi }}'
        if t == 'array':   return f'[ {len(value)} elementi ]'
        return str(value)
    ...
    def _tab_data(self, rows: list, data: dict) -> Gtk.Widget:
        ...
        if isinstance(rows[0], dict):
            headers = list(dict.fromkeys(
                k for row in rows[:20] if isinstance(row, dict) for k in row.keys()
            ))
        else:
