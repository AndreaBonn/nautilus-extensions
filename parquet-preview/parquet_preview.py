...
class ParquetPreviewWindow(Gtk.Window):
    ...
    def _build_content(self, data: dict):
        ...
        notebook.append_page(
            self._tab_metadata(data),
            Gtk.Label(label="ℹ Metadati")
        )
    ...
    def _tab_data(self, data: dict) -> Gtk.Widget:
        ...
        for i, h in enumerate(headers):
            ...
            type_lbl = Gtk.Label()
            type_lbl.set_markup(f"<span foreground='{dtype_color(col_type)}' size='small'>{col_type}</span>")
